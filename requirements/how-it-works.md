...
### Авто-генерация примеров (когда модели не согласны)

Помимо ручной коррекции преподавателем, система умеет **автоматически** создавать few-shot примеры. Это происходит, когда Claude и SLM дают разные вердикты:

```
SLM: "partially_correct"    Claude: "correct"
          │                         │
          └─── вердикты разошлись ───┘
                      │
                      ▼
         Авто-создание few-shot примера
         с вердиктом Claude и его explanation
```

Логика:
1. SLM оценивает ответ → `partially_correct`
2. Claude оценивает тот же ответ → `correct` + подробное объяснение
3. Система видит расхождение и автоматически создаёт запись в `prompt_examples`
4. В качестве `expected_verdict` используется вердикт Claude (как более надёжной модели)
5. В качестве `reason` — объяснение Claude

Это позволяет системе **самостоятельно** накапливать коррекции без вмешательства преподавателя. В debug mode отображается уведомление: *"SLM and Claude disagree — few-shot example auto-created"*, а счётчик примеров обновляется.

Преподаватель по-прежнему может переопределить вердикт через Teacher Override — в этом случае создаётся ещё один пример.
...
### 6. Счётчик примеров
В верхней панели отображается количество активных few-shot примеров в промпте SLM. Растёт при каждом расхождении моделей или коррекции преподавателя.
...
5. **Feedback Loop** — цикл: ошибка SLM → коррекция (авто или преподавателем) → пример в промпте → улучшение
